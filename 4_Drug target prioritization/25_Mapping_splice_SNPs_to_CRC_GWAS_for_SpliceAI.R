...
res<-fread("data/CRC_TWAS/Finemapping_results.csv")
...
fwrite(res,"data/CRC_TWAS/Finemapping_results_CRC_GWAS_P.csv")
res<-res[res$crc=="yes"]

res<-select(res,splice_id,SNP)
res<-distinct(res)

fwrite(res,"data/CRC_TWAS/SNP_list_for_SpliceAI.csv")

res<-fread("data/CRC_TWAS/SNP_list_for_SpliceAI.csv")
...
fwrite(res,"data/CRC_TWAS/SNPs_for_SpliceAI.csv")
