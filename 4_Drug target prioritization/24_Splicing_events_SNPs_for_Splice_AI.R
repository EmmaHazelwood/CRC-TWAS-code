...
events<-fread("data/CRC_TWAS/Colocalization_splicing_results_strong.csv")
...
as<-fread("/Splicing_GTEx/Adipose_Subcutaneous.v8.sqtl_allpairs_filtered.txt")
av<-fread("/Splicing_GTEx/Adipose_Visceral_Omentum.v8.sqtl_allpairs_filtered.txt")
ly<-fread("/Splicing_GTEx/Cells_EBV-transformed_lymphocytes.v8.sqtl_allpairs_filtered.txt")
cs<-fread("/Splicing_GTEx/Colon_Sigmoid.v8.sqtl_allpairs_filtered.txt")
ct<-fread("/Splicing_GTEx/Colon_Transverse.v8.sqtl_allpairs_filtered.txt")
...
for (a in unique(events$id)){
  ...
  for (c in unique(mr$tissue)){
    ...
    ld<-ld_matrix_local(sqtl$rs_id_dbSNP151_GRCh38p7, with_alleles = TRUE, bfile="data/UK_Biobank_Reference/mergey",plink_bin=get_plink_exe())
...
fwrite(results,"data/CRC_TWAS/Finemapping_results.csv")


# For missing events ------------------------------------------------------

results<-fread("data/CRC_TWAS/Finemapping_results.csv")
...
results2<-fread("data/CRC_TWAS/Finemapping_results.csv")
