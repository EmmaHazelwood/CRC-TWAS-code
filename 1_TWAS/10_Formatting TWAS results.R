...
fwrite(strong_all,"SMultiXcan_Expression_All_Strong.csv")
fwrite(weak_all,"SMultiXcan_Expression_All_Weak.csv")
fwrite(all_res,"SMultiXcan_Expression_All.csv")


# JTI ---------------------------------------------------------------------
setwd("/working/lab_tracyo/emmaHa/JTI_results/")
all<-list.files()
...
fwrite(strong_all,"JTI_All_Strong.csv")
fwrite(weak_all,"JTI_All_Weak.csv")
fwrite(all_res,"JTI_All.csv")


# Splicing ----------------------------------------------------------------

setwd("/working/lab_tracyo/emmaHa/SMultiXcan_Results/")
all<-list.files()
all<-all[grepl("splicing",all)]
...
as<-fread("GTEx_Analysis_v8_sQTL_groups/Adipose_Subcutaneous.leafcutter.phenotype_groups.txt",header = F)
av<-fread("GTEx_Analysis_v8_sQTL_groups/Adipose_Visceral_Omentum.leafcutter.phenotype_groups.txt",header = F)
ly<-fread("GTEx_Analysis_v8_sQTL_groups/Cells_Cultured_fibroblasts.leafcutter.phenotype_groups.txt",header = F)
cs<-fread("GTEx_Analysis_v8_sQTL_groups/Colon_Sigmoid.leafcutter.phenotype_groups.txt",header = F)
ct<-fread("GTEx_Analysis_v8_sQTL_groups/Colon_Transverse.leafcutter.phenotype_groups.txt",header = F)
wb<-fread("GTEx_Analysis_v8_sQTL_groups/Whole_Blood.leafcutter.phenotype_groups.txt",header = F)
...
fwrite(strong_all,"SMultiXcan_Splicing_Expression_All_Strong.csv")
fwrite(weak_all,"SMultiXcan_Splicing_Expression_All_Weak.csv")
fwrite(all_res,"SMultiXcan_Splicing_Expression_All.csv")

# Crossovers --------------------------------------------------------------

jti<-fread("Results/JTI_All_Strong.csv")
exp<-fread("SMultiXcan_Expression_All_Strong.csv")
spl<-fread("SMultiXcan_Splicing_Expression_All_Strong.csv")
