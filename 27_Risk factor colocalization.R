...
# Get analyses needed -----------------------------------------------------
list<-fread("CRC_TWAS/Consistent_all_3.csv")
...
list$id<-paste(list$gene,list$t_i_best,sep=";")
...
list<-list[grep("male",list$subtype),]

female<-list[grep("female",list$subtype),]
male<-list[!grep("female",list$subtype),]
  
female$id<-paste(female$gene,female$t_i_best,sep=";")
female<-dplyr::select(female,id)
female<-unique(female)

male$id<-paste(male$gene,male$t_i_best,sep=";")
male<-dplyr::select(male,id)
male<-unique(male)  
...
for (a in female$id){
  ...
  outcome_dat <- read_outcome_data(
    snps = exposure_dat$SNP,
    filename = "data/CRC_risk_factors/fat-distn.giant.ukbb.meta-analysis.whr.male_formatted.txt",
    sep = "\t",
    snp_col = "SNP",
    beta_col = "Effect",
    effect_allele_col = "Allele1",
  ...
  outcome_dat$samplesize.outcome<-381152
  outcome_dat$p_one_sided<-outcome_dat$pval.outcome/2
  outcome_dat$z<-qnorm(outcome_dat$p_one_sided)
  outcome_dat$se.outcome<-as.numeric(abs(outcome_dat$beta.outcome/outcome_dat$z))
  ...
  df3 <- data.frame(t(res$summary))
  df3$id<-a
  df3$risk_factor<-"WHR_female"
  results<-rbind(results,df3)

  #BMI
  outcome_dat <- read_outcome_data(
    snps = exposure_dat$SNP,
    filename = "data/CRC_risk_factors/ieu-a-974.csv",
    sep = ",",
    snp_col = "SNP",
    beta_col = "beta.exposure",
    se_col = "se.exposure",
...
for (a in male$id){
  ...
  outcome_dat$samplesize.outcome<-316772
  outcome_dat$p_one_sided<-outcome_dat$pval.outcome/2
  outcome_dat$z<-qnorm(outcome_dat$p_one_sided)
  outcome_dat$se.outcome<-as.numeric(abs(outcome_dat$beta.outcome/outcome_dat$z))
  ...
  df3 <- data.frame(t(res$summary))
  df3$id<-a
  df3$risk_factor<-"WHR_male"
  results<-rbind(results,df3)
  
  #BMI
  outcome_dat <- read_outcome_data(
    snps = exposure_dat$SNP,
    filename = "data/CRC_risk_factors/ieu-a-785.csv",
    sep = ",",
    snp_col = "SNP",
    beta_col = "beta.exposure",
    se_col = "se.exposure",
...
fwrite(results,"CRC_TWAS/Risk_factor_colocalization_results.csv")
